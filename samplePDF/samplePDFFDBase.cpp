...
samplePDFFDBase::samplePDFFDBase(std::string mc_version, covarianceXsec *xsec_cov)
    : samplePDFBase()
// DB Throughout constructor and init, pot is livetime for atmospheric samples
{
  ...
  if (xsec_cov == NULL)
  {
    std::cerr << "[ERROR:] You've passed me a NULL xsec covariance matrix... I need this to setup splines!" << std::endl;
    throw;
  }
...
void samplePDFFDBase::fillArray()
{
  ...
  for (unsigned int iSample = 0; iSample < MCSamples.size(); iSample++)
  {
    for (int iEvent = 0; iEvent < MCSamples[iSample].nEvents; iEvent++)
    {
      ...
      std::cout<<"Filling : "<<XBinToFill<<" with "<<XVar<<" | Filled Y Bin : "<<YBinToFill<<std::endl;
    }
  }
    ...
    for (int iEvent = 0; iEvent < MCSamples[iSample].nEvents; iEvent++)
      {
        ...
        if (XBinToFill != -1 && YBinToFill != -1)
        {
          // std::cout << "Filling samplePDFFD_array at YBin: " << YBinToFill << " and XBin: " << XBinToFill << " | X Variable Value : "<<XVar << std::endl;
        ...
        // std::cout<<"Filling : "<<XBinToFill<<" with "<<XVar<<std::endl;
      }
    }
...
#ifdef DEBUG
    if (TMath::IsNaN(xsecw))
      std::cout << "iParam=" << iParam << "xsecweight=nan from norms" << std::endl;
#endif
  }
  return xsecw;
}
...
void samplePDFFDBase::set1DBinning(std::vector<double> &XVec)
{
  ...
  double* YBinEdges_Arr = YBinEdges.data();
...
void samplePDFFDBase::addData(std::vector<double> &data)
{
  ...
  if (GetNDim() != 1)
  {
    std::cerr << "Trying to set a 1D 'data' histogram in a 2D sample - Quitting" << std::endl;
    throw;
  }
...
void samplePDFFDBase::addData(std::vector<vector<double>> &data)
{
  ...
  if (GetNDim() != 2)
  {
    std::cerr << "Trying to set a 2D 'data' histogram in a 1D sample - Quitting" << std::endl;
    throw;
  }
  ...
  for (int i = 0; i < (int)MCSamples.size(); ++i)
  {
    ...
    for (int j = 0; j < MCSamples[i].nEvents; ++j)
    {
      ...
      case 2:
        ...
        EventSplines = splineFile->GetEventSplines(GetSampleName(), i, *(MCSamples[i].mode[j]), *(MCSamples[i].rw_etru[j]), *(MCSamples[i].x_var[j])*1000, *(MCSamples[i].y_var[j]));
